...
abstract class AbstractClsStubBuilderTest : LightCodeInsightFixtureTestCase() {
    fun doTest(sourcePath: String) {
        val ktFileText = File("$sourcePath/${lastSegment(sourcePath)}.kt").readText()
        val jvmFileName = InTextDirectivesUtils.findStringWithPrefixes(ktFileText, "JVM_FILE_NAME:")
